# EndOfDayReport.gd
# Attach to the EndOfDayReport PanelContainer in Main.tscn
# Shows the consequences of the player's decisions and the Public Trust reveal.
...
func show_report(report: Array, day: int, time_expired: bool = false) -> void:
	...
	# ── Public Trust Summary (shown at top of report) ──
	...
	# Color the trust label based on trust level
